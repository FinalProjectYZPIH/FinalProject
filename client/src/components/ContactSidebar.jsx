  ...
  return (
    ...
    >
      <div className={`h-4/5 mt-5  mx-2 overflow-x-hidden rounded-xl shadow-lg  border border-cyan-400 p-1 flex flex-col items-center ${color}`}>
        ...
        {contact?.map((friend) => (
          <div
            className={`w-3/4 border border-cyan-400 p-2 mx-1 my-1 rounded-lg text-left hover:bg-cyan-400 hover:bg-opacity-50 ${friend.Online ? "text-green-500" : "text-red-500"} ${color} `}
            key={friend.username}
          >
            {friend?.toUpperCase()}
            {/* ( {contact.Online ? '✅️' : '❌️'}) */}
